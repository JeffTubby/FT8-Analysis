...
def get_my_data_for_band(band_type): #gmd
...
def main():
    ...
    band_type = str(band_type).strip().lower()


    df = get_my_data_for_band(band_type)
    ...
    plt.title(f"World Map with Continents QSO by {band_type} band", fontsize=16)
    plt.tight_layout()
    plt.show()
