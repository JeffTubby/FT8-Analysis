...
def load_year_data(year_type: int) -> pd.DataFrame:
    """loads the data"""
...
def main():
    ...
    print(f"Band selected:  Year selected: {year_type}")
    ...
    plt.title(f'Monthly SFI for year {year_type}', fontsize=10)
    ...
    plt.title(f'FT8 RST Analysis for Year {year_type}', fontsize=10)
    data['DATE'] = pd.to_datetime(data['DATE'], errors='coerce')
    ...
    sns.set_style("darkgrid")
    plt.figure(figsize=(5,3))
    plt.title(f'FT8 mean RST for Year {year_type}')
    data['DATE'] = pd.to_datetime(data['DATE'], errors='coerce')
